    ...
    fn mapping_insert_anchor(&self, path: &str) -> Result<(usize, usize, usize)> {
        ...
        let anchor = entries
            ...
            .find_map(|((key_start, key_end), child_tree)| {
                ...
                // Two kinds of entry whose *value* span is not the
                // entry's own bytes, both anchored on the key's line:
                //
                // * an implicit null (`b:` with no value) owns no value
                //   bytes at all;
                // * an alias (`b: *d`) resolves through to the anchor's
                //   definition, which is somewhere else entirely — and
                //   taking that span put the new key beside the
                //   *anchor*, in a different mapping, at `Ok(())`.
                //
                // The column comes from the key directly in both cases:
                // `column_of_key_at` infers a key's column from a
                // *value* offset, so it would walk past these.
                if start == end || matches!(child_tree, SpanTree::Alias(_)) {
