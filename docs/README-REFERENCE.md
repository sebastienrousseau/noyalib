...
# Detailed README reference

This document preserves the extended technical material from the primary README
before the workspace-wide canonical README template was adopted. The concise
[`README.md`](../README.md) is the project entry point. This reference retains
the deeper rationale, feature catalogue, security discussion, and examples so
the template migration does not discard documentation.
...
- [Ecosystem comparison](#ecosystem-comparison) — short matrix; full table at [`docs/COMPARISON.md`](COMPARISON.md)
- [Benchmarks](#benchmarks) — headline numbers; full table at [`docs/BENCHMARKS.md`](BENCHMARKS.md)
...
```toml
[dependencies]
noyalib = { version = "0.0.46", default-features = false }
```
...
| `lossless-u64` | — | `Number::Unsigned(u64)` plus opt-in parser/serializer config for scalars in `(i64::MAX, u64::MAX]` | [`docs/adr/0004-lossless-u64-integers.md`](adr/0004-lossless-u64-integers.md), `examples/lossless_u64.rs`, `benches/lossless_u64.rs` |
...
| `noyavalidate` | `std` + `miette` + `validate-schema` | Enables the schema-validation surface the `noyavalidate` CLI is built on | [`docs/USER-GUIDE.md` §11](USER-GUIDE.md) |

```toml
# Example: rich diagnostics + schema validation
[dependencies]
noyalib = { version = "0.0.46", features = ["miette", "validate-schema"] }
```
...
[`docs/adr/0004-lossless-u64-integers.md`](adr/0004-lossless-u64-integers.md)
...
at release time — see
[`docs/POLICIES.md` §1](POLICIES.md#1-msrv-minimum-supported-rust-version).
...
([ADR-0005](adr/0005-workspace-split.md)) — the version number
...
- **MCP**: [`sebastienrousseau/noyalib-mcp`](https://github.com/sebastienrousseau/noyalib-mcp) — tools, host configs (split repo since v0.0.13; strict lockstep per [ADR-0005](adr/0005-workspace-split.md)).
...
[`docs/MIGRATION-FROM-SERDE-YAML.md`](MIGRATION-FROM-SERDE-YAML.md).
...
|---|---|---|---|
| [`serde_yml`](https://crates.io/crates/serde_yml) | `0.0.13` (repo archived) | mostly | [`MIGRATION-FROM-SERDE-YML.md`](MIGRATION-FROM-SERDE-YML.md) |
...
| [`yaml-spanned`](https://crates.io/crates/yaml-spanned) | `0.0.3` | **no** (parser-only, no `to_string`) | [`MIGRATION-FROM-YAML-SPANNED.md`](MIGRATION-FROM-YAML-SPANNED.md) |

The umbrella index is
[`docs/MIGRATION.md`](MIGRATION.md) — start there if you're
...
**[`docs/COMPARISON.md`](COMPARISON.md)** so the README
stays fast to scan.
...
stack, etc.) live in
[`POLICIES.md` §4](POLICIES.md#4-performance--algorithmic-complexity).
...
| **Values** | 7-variant `Value` enum: Null, Bool, Number, String, Sequence, Mapping, Tagged. Path traversal via `get_path("server.host")`. Path queries via `query("items[*].name")` with wildcards (`*`), recursive descent (`..`), and bracket-quoted keys (`labels["app.kubernetes.io/name"]`) for a key the grammar would otherwise read as structure; `noyalib::path::join_keys` spells such a path from literal keys. Deep merge via `merge()` and `merge_concat()`. `MappingAny` for non-string keys. Zero-copy `BorrowedValue<'a>` borrows strings from input (18% faster). |
...
- **You have a hard dependency budget that cannot tolerate a
  ...
  { version = "0.0.46", default-features = false, features =
...
- **[Ecosystem map](ECOSYSTEM.md)** — the six crates, the lockstep model, the scorecard

| Document | Covers |
|---|---|
| [`docs/POLICIES.md`](POLICIES.md) | MSRV, SemVer & API stability, security & audit pipeline, performance & algorithmic complexity, concurrency guarantees, platform support, the full feature-flag matrix, panic policy, error model, dependency policy, release & changelog policy. **Single source of truth** for engineering posture. |
| [`docs/MIGRATION.md`](MIGRATION.md) | Umbrella index of migration guides for `serde_yaml` 0.9, `serde_yml`, `yaml_serde`, `serde-yaml-ng`, `serde-norway`, `serde-yaml-bw`, `serde-saphyr`, `yaml-spanned`. Each linked guide is a per-crate function-mapping table + behavioural-difference notes + checklist. |
...
| [`docs/release-notes/`](release-notes/README.md) | Narrative notes for v0.0.1–v0.0.17, one file per tag. Complements the changelog rather than duplicating it. |
| [`docs/USER-GUIDE.md`](USER-GUIDE.md) | Long-form usage guide with worked examples for every major feature. |
| [`docs/ARCHITECTURE.md`](ARCHITECTURE.md) | Module map, hot-path notes, design decisions. |
| [`docs/COOKBOOK.md`](COOKBOOK.md) | Task-shaped recipes, each pointing at the runnable example it comes from. |
| [`docs/GLOSSARY.md`](GLOSSARY.md) | YAML / serde terminology reference. |
| [`docs/internals.md`](internals.md) | Library internals (parser stages, loader frames, CST green tree). |
| [`docs/errors.md`](errors.md) | Error reference — every variant, when it fires, how to handle it. |
...
- **MSRV.** Raised only on the breaking axis with the reason
  ...
  in [`doc/MSRV-AND-DEPRECATION.md`](MSRV-AND-DEPRECATION.md).
